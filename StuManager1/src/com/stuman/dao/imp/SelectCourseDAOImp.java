...
public class SelectCourseDAOImp implements SelectCourseDAO {
	...
	public List listSelectCourseBySno(String sno) {
		try {
			// 获得Session
			...
			// 执行操作
			List results = s.createQuery("from Selectcourse as cour where cour.id.sno=:stuNo").setString("stuNo", sno).list();
			// 提交事务
	...
	public List listSelectCourseByCno(String cno) {
		try {
			// 获得Session
			Session s = HibernateUtil.currentSession();
			// 开始事务
			HibernateUtil.beginTransaction();
			// 执行操作
			List results = s.createQuery("from Selectcourse as cour where cour.id.cno=:CourNo").setString("CourNo", cno).list();
			// 提交事务
			HibernateUtil.commitTransaction();
			// 关闭Session
			HibernateUtil.closeSession();
